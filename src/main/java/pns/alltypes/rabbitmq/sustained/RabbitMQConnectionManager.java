...
public class RabbitMQConnectionManager implements Serializable {
   ...
   public synchronized static RabbitMQConnectionManager getInstance(final RabbitConnectionConfig connectionConfig) {
      ...
      if (rabbitMQConnectionManager != null) {
         ...
         RabbitMQConnectionManager.RABBIT_CONN_MANAGER_TO_CONN_CONFIG.put(connectionConfig, rabbitMQConnectionManager);
         return rabbitMQConnectionManager;
      }
   ...
   public AmqpConnection selectConnection() {
      if (LOGGER.isTraceEnabled()) {
         LOGGER.trace("Going to select a connection");
      }
   ...
   public void destroy() {
      if (LOGGER.isTraceEnabled()) {
         LOGGER.trace("Destroying rabbitmq connection manager");
      }
   }
   ...
   public void addConnection() {
      ...
      if (LOGGER.isTraceEnabled()) {
         LOGGER.trace("Going to create connection with every connection having another backup");
      }
      createConnection();
      createConnection();
   ...
   public void createChannel() {
      if (LOGGER.isTraceEnabled()) {
         LOGGER.trace("Going to create channel in a delay");
      }
   ...
   public void createConnection() {
      if (LOGGER.isTraceEnabled()) {
         LOGGER.trace("Going to create connection in a delayed way");
      }
   ...
   public AmqpChannel closeAndReopenChannel(final AmqpChannel channel) {
      if (LOGGER.isTraceEnabled()) {
         LOGGER.trace(String.format("Going to close and get a new channel for %s",channel));
      }
      if (channel != null) {
         channel.close();
      }
   ...
   public void hintResourceAddition() {
      if (LOGGER.isTraceEnabled()) {
         LOGGER.trace("somone hinting that a new connection and a channel is required");
      }
      createConnection();
      createChannel();
