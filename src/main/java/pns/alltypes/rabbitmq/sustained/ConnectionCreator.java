...
public class ConnectionCreator implements Runnable {
   ...
   private void establishConnection() throws IOException {
      ...
      final ConnectionFactory connectionFactory = rabbitMQConnectionManager.getConnectionConfig()
            .getConnectionFactory();
      final Connection connection = rabbitMQConnectionManager.getConnectionConfig()
            .getHighAvailabilityHosts().length == 0
                  ? connectionFactory.newConnection()
                  : connectionFactory
                        .newConnection(rabbitMQConnectionManager.getConnectionConfig().getHighAvailabilityHosts());
      ...
      connection.addShutdownListener(new ShutdownListener() {
         ...
         public void shutdownCompleted(final ShutdownSignalException cause) {
            ...
            try {
               ...
               ConnectionCreator.LOGGER.error(String.format("Error occured on shutdown of connection  %s", e));

            }
            finally {
               // dont care for other exception . go and create next connection and channel
               if (LOGGER.isTraceEnabled()) {
                  LOGGER.trace("Going to create connection after connection shutdown");
               }
               rabbitMQConnectionManager.createConnection();
            }
